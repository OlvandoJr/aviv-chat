// boleto-link — link PÚBLICO e estável do boleto (sem login). O cliente abre pelo
// WhatsApp: GET ?t=<public_token> → gera uma signed URL fresca do PDF (bucket privado
// "boletos") e redireciona (302). Sem PDF, mostra uma página simples com os dados que
// temos (linha digitável + valor + vencimento). Token aleatório = não enumerável.
// Deploy: --no-verify-jwt (público).
...
Deno.serve(async (req) => {
  ...
  if (!token) return html('<h2>Link inválido</h2><p>Token do boleto ausente.</p>', 400)
