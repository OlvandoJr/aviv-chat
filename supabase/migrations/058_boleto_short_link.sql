-- 058_boleto_short_link.sql
-- Encurta o link do boleto: troca o UUID (public_token, 36 chars) por um
-- short_code (8 chars) e a função `boleto-link` pela função curta `b`.
--   antes:  /functions/v1/boleto-link?t=<uuid>
--   depois: /functions/v1/b?c=<short_code>
-- Links antigos seguem funcionando (a edge resolve ?t e ?c).
-- IMPORTANTE: manter WITH (security_invoker = true) nas views (regressão do #60).
...
UPDATE public.boletos_emitidos
   SET short_code = substr(md5(random()::text || id::text || clock_timestamp()::text), 1, 8)
 WHERE short_code IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_boletos_emitidos_short_code
  ON public.boletos_emitidos(short_code);

-- ── Views de cobrança: URL curta ────────────────────────────────────────────
...
WITH (security_invoker = true) AS
 WITH sienge AS (
         SELECT DISTINCT ON (sb.phone_norm) sb.phone_norm, 'sienge'::text AS source, sb.customer_name, sb.customer_phone,
            ...
            CASE WHEN be.short_code IS NOT NULL
                 THEN 'https://jpxlczmbxfcnujemlxzq.supabase.co/functions/v1/b?c=' || be.short_code
