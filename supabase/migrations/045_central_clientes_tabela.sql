-- 045_central_clientes_tabela.sql
-- Tabela profissional da Central de Clientes: vw_central_clientes ganha 3 colunas
-- novas (ao final, p/ CREATE OR REPLACE): email, contrato_situacao, boleto_status.
--  • email: coluna nova em sienge_clientes (nula por ora; sync do Sienge vem depois).
--  • contrato_situacao: situação do contrato (sienge_contratos via vw_cliente_contrato).
--  • boleto_status: pago | vencido | enviado | a_enviar | sem_boleto (regra abaixo).

ALTER TABLE public.sienge_clientes ADD COLUMN IF NOT EXISTS email text;
...
CREATE OR REPLACE VIEW public.vw_central_clientes AS
 WITH phones AS (
         ...
         SELECT DISTINCT ON ((normalize_phone(chat_contacts.wa_id))) normalize_phone(chat_contacts.wa_id) AS phone_norm,
           ...
           FROM chat_contacts
          WHERE normalize_phone(chat_contacts.wa_id) IS NOT NULL
         ...
         SELECT DISTINCT ON (sienge_clientes.phone_norm) sienge_clientes.phone_norm,
          ...
          WHERE sienge_clientes.phone_norm IS NOT NULL
         ...
         SELECT normalize_phone(cc.wa_id) AS phone_norm,
            bool_or(co.status = 'open'::text) AS conversa_aberta,
           ...
           FROM chat_contacts cc JOIN chat_conversations co ON co.contact_id = cc.id
         ...
         SELECT vw_clientes_boletos.phone_norm, vw_clientes_boletos.source AS boleto_source,
            ...
            vw_clientes_boletos.amount AS boleto_valor
           FROM vw_clientes_boletos
        ), sgl AS (
         SELECT mensagens_cobranca.phone_norm, max(mensagens_cobranca.pessoanomecompleto) AS sgl_nome,
            count(*) AS sgl_registros, max(mensagens_cobranca.app_dispatched_at) AS ultima_sgl,
            count(*) FILTER (WHERE mensagens_cobranca.app_dispatched_at IS NOT NULL) AS sgl_enviadas
           FROM mensagens_cobranca WHERE mensagens_cobranca.phone_norm IS NOT NULL
          GROUP BY mensagens_cobranca.phone_norm
        ), regua AS (
        ...
        ), pago AS (   -- pagamento recente (últimos 40 dias), p/ status "pago" quando não há boleto em aberto
         SELECT x.phone_norm, max(x.paid_at) AS ultimo_pago FROM (
                ...
                UNION ALL
                SELECT phone_norm, paid_at FROM sienge_boletos    WHERE lower(COALESCE(status,'')) = 'pago' AND paid_at IS NOT NULL
              ) x
          WHERE x.phone_norm IS NOT NULL GROUP BY x.phone_norm
        ), em AS (   -- boletos EMITIDOS em aberto (inclui cliente novo sem parcela em sienge_boletos, ex.: avulso)
         SELECT phone_norm,
 ...
 SELECT p.phone_norm,
    ...
    ct.profile_picture_url,
        CASE
            WHEN s.phone_norm IS NOT NULL AND sg.phone_norm IS NOT NULL THEN 'ambos'::text
            WHEN s.phone_norm IS NOT NULL THEN 'sienge'::text
            WHEN sg.phone_norm IS NOT NULL THEN 'sgl'::text
            ELSE 'contato'::text
        END AS origem,
    ...
    cv.conversation_id,
    COALESCE(cv.conversas, 0::bigint) AS conversas,
    cv.last_message_at,
    COALESCE(mg.msgs_enviadas, 0::bigint) AS msgs_enviadas,
    COALESCE(mg.msgs_total, 0::bigint) AS msgs_total,
    ...
    s.email,
    cc2.situation AS contrato_situacao,
    CASE
        ...
        WHEN pg.ultimo_pago IS NOT NULL
             AND pg.ultimo_pago >= (now() - interval '40 days') THEN 'pago'::text
